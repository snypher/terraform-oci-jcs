...
# This JCS instance will have a Load Balancer
resource "oraclepaas_java_service_instance" "jcs_priv" {
  name = "${var.jcs_name}"
  ...
  count = "${(var.jcs_create_lb == true) ? 1 : 0}"
}

# Create the Java Cloud Service (JCS) instance for this environment
# The JCS instance will reside in the application compartment
# This JCS instance will not have a Load Balancer
resource "oraclepaas_java_service_instance" "jcs_pub" {
  name = "${var.jcs_name}"
  description = "Java Cloud Service (JCS) instance for ${var.environment} environment ${var.app_tag} project"
  edition = "${var.jcs_edition}"
  service_version = "${var.jcs_service_version}"
  ...
  count = "${(var.jcs_create_lb == false) ? 1 : 0}"
}
